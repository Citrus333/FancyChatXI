...
utils.RepairSettings = function(t1, t2, seen)
	...
	-- Recurse into subtables, fix type mismatches, AND backfill keys
	-- that are entirely missing from t2.  That last case matters
	-- whenever an addon update adds a brand-new setting: t2 (the
	-- user's already-saved file, from before the update) simply
	-- doesn't have the key yet, so v2 is nil.  Every branch below
	-- used to require v2 ~= nil, so a missing key silently stayed
	-- nil forever instead of getting the default - any code reading
	-- it (e.g. allSettings.HideDuringCutscenes[1]) then throws
	-- "attempt to index nil value" every frame, permanently.
	--
	-- Without the mismatch repair, a key that was a plain
	-- number/string in an older version of the addon but is now a
	-- table (or vice versa) in defaults will survive in t2 with its
	-- old type.  Sugar's table.merge then crashes during
	-- character-switch with `rawget(number, k)` because it tries to
	-- recurse into the number expecting a table — see the logout
	-- traceback.  Fixing the mismatch by replacing t2's value with a
	-- clone of the default keeps the file readable by the next merge.
	for k, v1 in pairs(t1) do
		local v2 = t2[k]
		if v2 == nil then
			t2[k] = (type(v1) == 'table') and utils.cloneTable(v1) or v1
