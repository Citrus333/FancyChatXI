...
function M.default_fcw()
	return T{
		T{
			...
			-- HideChat is the COMBINED/effective hide state - every
			-- visibility/opacity check throughout render.lua reads this
			-- one.  It's recomputed once per frame (right after par.InEvent
			-- is read) as `UserHideChat or (cutscene auto-hide)`, so it
			-- always reflects "should FancyChat be hidden right now" for
			-- whatever reason.
			HideChat              = false,
			-- UserHideChat is the user's own manual intent (the hide-chat
			-- shortcut in input.lua toggles THIS, not HideChat directly)
			-- so a cutscene ending doesn't un-hide chat the user hid
			-- themselves, and manually un-hiding mid-cutscene doesn't
			-- fight the auto-hide until the cutscene actually ends.
			UserHideChat          = false,
			-- Typing-preview caret tracking (see input.lua's key_state
			-- callback and render.lua's typing-preview draw block).
			-- TypingCursorPos is the tracked caret index into the raw
			-- input text; TypingKeyDown holds per-key edge-detection
			-- state for Left/Right/Home/End so a held key doesn't
			-- re-trigger every single frame.
			TypingCursorPos       = 0,
			TypingCursorWasOpen   = false,
			TypingCursorPrevLen   = 0,
			TypingKeyDown         = {},
			PrevHideChat          = false,
			PrevAnchor_X          = -1,
			PrevAnchor_Y          = -1,
...
function M.default_settings()
	return T{
		...
		-- When true, FancyChat auto-hides itself whenever the native
		-- interface is hidden (uiw.UIVisiblePtr + 0xB4) and reappears
		-- the moment it isn't - this covers a real letterboxed cutscene
		-- AND the player's own "hide UI" hotkey, since both set the
		-- same native flag.  Purely automatic; does not affect or get
		-- affected by the manual hide-chat shortcut (fcw1.UserHideChat)
		-- - see the combine step in render.lua.
		HideDuringCutscenes  = T{true},
		HelpButton           = T{true},
		CompactTabs          = false,
		PlayerName           = '---',
		FormatTSMode         = 1,
