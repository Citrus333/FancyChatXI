...
-- ===================================================================
-- TEMP DIAGNOSTIC — menu-position investigation.
--
-- Hex-dumps 0x100 bytes from each of the native log window structs
-- (WinPtr1 / WinPtr2) to a text file.  Run once with the log visible
-- (FORCE_CLOSE=false in render.lua, log settled open) and once with
-- it forced closed (FORCE_CLOSE=true, or just close it manually),
-- then diff the two files.  We already know +0x34 is the field that
-- drives the menu anchor (0 = collapsed/wrong position, 0x0F3C/3900
-- = correct position, but also fully visible).  We're looking for a
-- DIFFERENT offset that changes between visible/hidden — a candidate
-- for hiding the window without touching +0x34.
-- ===================================================================
local function dump_ptr(f, label, base_ptr)
	f:write('== '..label..' (base='..string.format('0x%08X', base_ptr)..') ==\n')
	if base_ptr == 0 then
		f:write('  (null pointer)\n\n')
		return
	end
	for row = 0, 0x0F do
		local line = string.format('+0x%02X: ', row * 16)
		for col = 0, 15 do
			local off = row * 16 + col
			local ok, byte = pcall(ashita.memory.read_uint8, base_ptr + off)
			line = line..(ok and string.format('%02X ', byte) or '?? ')
		end
		f:write(line..'\n')
	end
	f:write('\n')
...
function M.register()
	ashita.events.register('command', 'command_cb', function (e)
		...
		if not fcw[1].Closing and fcw[1].InitDone and fcw[1].LoggedIn then
			...
			if (#args == 2 and args[2] == 'cutscenehide') then
				allSettings.HideDuringCutscenes[1] = not allSettings.HideDuringCutscenes[1]
				print('FancyChat: auto-hide during cutscenes / native UI-hide '
					..(allSettings.HideDuringCutscenes[1] and 'ENABLED' or 'DISABLED'))
				SaveSettings()
				return
			end
			if (#args == 2 and args[2] == 'dumpwin') then
				local ts   = os.date('%Y_%m_%d-%H_%M_%S', os.time())
				local path = addon.path..'/dumpwin_'..ts..'.txt'
				local f    = io.open(path, 'w')
				if not f then
					print('FancyChat: could not open '..path)
					return
				...
				local ptr1 = ashita.memory.read_uint32(uiw.WinPtr1)
				local ptr2 = ashita.memory.read_uint32(uiw.WinPtr2)
				dump_ptr(f, 'WinPtr1', ptr1)
				dump_ptr(f, 'WinPtr2', ptr2)

				f:close()
				print('FancyChat: dumped WinPtr1/WinPtr2 (0x100 bytes each) to '..path)
				return
			end
