...
function M.register()
	...
	ashita.events.register('key_state', 'key_state_callback1', function (e)
		...
		-- ---------------------------------------------------------------
		-- Typing-preview caret tracking (render.lua's live mirror of the
		-- native chat input reads fcw[1].TypingCursorPos to know where
		-- to draw the blinking cursor).  Ashita doesn't expose the
		-- game's actual caret index, so we track our own:
		--   - Left/Right/Home/End move it explicitly (edge-triggered,
		--     single step per press - no auto-repeat on hold yet).
		--   - Plain typing / Backspace / Delete are inferred from how
		--     the raw text's LENGTH changes frame-to-frame, since we
		--     don't hook every possible printable key individually.
		--     Typing and Backspace both move the caret with the text;
		--     Delete removes the character AFTER the caret, so the
		--     caret itself doesn't move - told apart by whether Delete
		--     is currently held when the length shrinks.
		-- Resets to the end of the text whenever the input box just
		-- opened this frame, and is clamped to the text bounds every
		-- frame as a safety net (e.g. history-cycling replaces the
		-- whole line at once, which isn't a plain length delta).
		-- ---------------------------------------------------------------
		if AshitaCore:GetChatManager():IsInputOpen() == 0x11 then
			local typedText = AshitaCore:GetChatManager():GetInputTextRaw()
			local typedLen  = typedText and #typedText or 0

			if not fcw[1].TypingCursorWasOpen then
				fcw[1].TypingCursorPos = typedLen
			end
			fcw[1].TypingCursorWasOpen = true

			if fcw[1].TypingCursorPos == nil then
				fcw[1].TypingCursorPos = typedLen
			end

			local prevLen = fcw[1].TypingCursorPrevLen or typedLen
			if typedLen ~= prevLen then
				if keyptr[211] ~= 0 then
					-- Delete held: text shrank in front of the caret,
					-- caret position itself doesn't move.
				else
					-- Typing (grew) or Backspace (shrank): caret
					-- follows the length delta 1:1 in the common case.
					fcw[1].TypingCursorPos = fcw[1].TypingCursorPos + (typedLen - prevLen)
				end
			end
			fcw[1].TypingCursorPrevLen = typedLen

			local function typingKeyEdge(id, scancode)
				local down = keyptr[scancode] ~= 0
				local was  = fcw[1].TypingKeyDown[id]
				fcw[1].TypingKeyDown[id] = down
				return down and not was
			end
			if typingKeyEdge('Left', 203) then
				fcw[1].TypingCursorPos = math.max(0, fcw[1].TypingCursorPos - 1)
			end
			if typingKeyEdge('Right', 205) then
				fcw[1].TypingCursorPos = math.min(typedLen, fcw[1].TypingCursorPos + 1)
			end
			if typingKeyEdge('Home', 199) then
				fcw[1].TypingCursorPos = 0
			end
			if typingKeyEdge('End', 207) then
				fcw[1].TypingCursorPos = typedLen
			end

			if fcw[1].TypingCursorPos > typedLen then fcw[1].TypingCursorPos = typedLen end
			if fcw[1].TypingCursorPos < 0 then fcw[1].TypingCursorPos = 0 end
		else
			fcw[1].TypingCursorWasOpen = false
		end
		...
		if allSettings.shortcutHideEnabled[1]
			...
			fcw[1].UserHideChat = not fcw[1].UserHideChat
			ResetAutoHideTimer()
			SetChatOpacity(1, 1)
