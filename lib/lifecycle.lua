...
function M.register()
	...
	ashita.events.register('load', 'load_cb', function ()
		...
		-- Live typing preview - mirrors the native chat input box.
		-- Styled a bit dimmer than normal chat text and prefixed with
		-- a caret in the update code (render.lua) so it visually
		-- reads as "in progress", not a sent line.  Starts hidden;
		-- render.lua flips visibility based on IsInputOpen().
		fo.Typing = gdi:create_object(allSettings.fontSettings, false)
		fo.Typing:set_font_height(allSettings.fontSettings.font_height)
		fo.Typing:set_font_color(0xFFB0B0B0)
