...
function M.register()
	ashita.events.register('d3d_present', 'present_cb', function ()
		...
		if fcw1.LoggedIn and not fcw1.Closing and not fcw1.Zoning then
			...
			-- Combined/effective hide state.  Everything else in this
			-- file reads fcw1.HideChat, so this is the one place that
			-- decides whether FancyChat should be hidden right now:
			-- either the user hid it themselves (shortcut in input.lua),
			-- or the native interface is hidden and auto-hide is
			-- enabled - which covers BOTH a real cutscene AND the
			-- player's own "hide UI" hotkey, since both set the exact
			-- same flag (uiw.UIVisiblePtr + 0xB4).  Confirmed via
			-- onimitch's minimapcontrol addon, which reads this same
			-- pointer for the same reason.
			--
			-- par.InEvent alone isn't a usable signal here - it's also
			-- true for plain NPC dialogue (talking to any NPC engages
			-- the same event-lock the client uses for cutscenes), so it
			-- can't tell dialogue apart from a real cutscene by itself.
			-- A native-log peek approach was tried and also didn't work
			-- (something else writes nonzero noise into that field
			-- every frame once it's permanently forced closed).
			--
			-- isInterfaceHidden alone is NOT gated on par.InEvent
			-- anymore - the manual hide-UI hotkey wouldn't have an
			-- event active, so requiring both would miss it.
			--
			-- Real transitions still clear the debounce bar with room
			-- to spare; it only exists to smooth over single-frame
			-- blips right at a transition.
			local CUTSCENE_HIDE_DEBOUNCE = 0.05

			local isInterfaceHidden = false
			if uiw.UIVisiblePtr and uiw.UIVisiblePtr ~= 0 then
				local ifacePtr = ashita.memory.read_uint32(uiw.UIVisiblePtr + 10)
				if ifacePtr ~= 0 then
					isInterfaceHidden = ashita.memory.read_uint8(ifacePtr + 0xB4) == 1
				end
			end

			local DEBUG_CUTSCENE_PRINT    = false
			local DEBUG_CUTSCENE_THROTTLE = 0.25
			if DEBUG_CUTSCENE_PRINT then
				local sig = tostring(par.InEvent)..'|'..tostring(isInterfaceHidden)
				if sig ~= uiw.DebugCutsceneLastSig and (_now - (uiw.DebugCutsceneLastPrint or 0)) > DEBUG_CUTSCENE_THROTTLE then
					print(('[fchat-debug] InEvent=%s  isInterfaceHidden=%s')
						:format(tostring(par.InEvent), tostring(isInterfaceHidden)))
					uiw.DebugCutsceneLastSig   = sig
					uiw.DebugCutsceneLastPrint = _now
				end
			end

			if isInterfaceHidden then
				if not uiw.CutsceneCandidateSince then
					uiw.CutsceneCandidateSince = _now
				end
			else
				uiw.CutsceneCandidateSince = nil
			end
			local inCutsceneDebounced = uiw.CutsceneCandidateSince ~= nil
				and (_now - uiw.CutsceneCandidateSince) >= CUTSCENE_HIDE_DEBOUNCE

			fcw1.HideChat = fcw1.UserHideChat
				or ((allSettings.HideDuringCutscenes and allSettings.HideDuringCutscenes[1]) and inCutsceneDebounced)
		...
		-- Render All FancyChat windows --
			...
			-- Setting variables to position the chat window elements --
				...
				-- Live typing preview.  0x11 is specifically "player is
				-- typing a chat line" (same check input.lua's history
				-- capture uses) - excludes menu text boxes, search
				-- fields, etc.  ParseAutoTranslate converts the raw
				-- <AT> escape bytes GetInputTextRaw() returns into the
				-- readable bracketed phrase, same call parser.lua
				-- already uses for incoming messages.
				--
				-- The blinking '|' is drawn at fcw1.TypingCursorPos,
				-- input.lua's own tracked caret index (Ashita doesn't
				-- expose the game's real one).  Text is split at that
				-- raw byte offset and each half is run through
				-- ParseAutoTranslate SEPARATELY, so an <AT> phrase on
				-- either side of the cursor still renders as readable
				-- text instead of raw escape bytes.  Tracking is
				-- inferred from key presses / text-length deltas, so
				-- it can drift slightly right around an <AT> phrase
				-- specifically (those insert as a multi-byte block,
				-- not one char at a time) - it resyncs on normal typing.
				if AshitaCore:GetChatManager():IsInputOpen() == 0x11 and not fcw1.HideChat then
					local rawInput  = AshitaCore:GetChatManager():GetInputTextRaw() or ''
					local cursorIdx = math.min(math.max(fcw1.TypingCursorPos or #rawInput, 0), #rawInput)
					local rawBefore = rawInput:sub(1, cursorIdx)
					local rawAfter  = rawInput:sub(cursorIdx + 1)
					local cleanBefore = (#rawBefore > 0)
						and AshitaCore:GetChatManager():ParseAutoTranslate(rawBefore, true) or ''
					local cleanAfter = (#rawAfter > 0)
						and AshitaCore:GetChatManager():ParseAutoTranslate(rawAfter, true) or ''
					local cursorOn = (math.floor(_now * 2) % 2) == 0
					fo.Typing:set_text('> '..cleanBefore..(cursorOn and '|' or ' ')..cleanAfter)
					fo.Typing:set_visible(true)
					fo.Typing:set_position_x(fcw1.Anchor_X)
					fo.Typing:set_position_y(fcw1.Anchor_Y + allSettings.fontSettings.font_height)
				else
					fo.Typing:set_visible(false)
				end
